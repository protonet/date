...
parser.prototype.timeAgo = function() {
  ...
  if (captures = dateI18n.rAgo.exec(this.str)) {
    ...
    var methodName = dateI18n.agoMapping[mod];
...
parser.prototype.week = function() {
  var captures;
  if (captures = dateI18n.words.rWeeks.exec(this.str)) {
    this.skip(captures);
    return 'week';
  }
};
...
parser.prototype.houroclock = function() {
  var captures;
  if (captures = dateI18n.rHourOclock.exec(this.str)) {
    this.skip(captures);
...
parser.prototype.nextTime = function(before) {
  ...
  if (before <= d.date || dateI18n.rPast.test(orig)) {
    return this;
  }
  // If time is in the past, we need to guess at the next time
  if (dateI18n.rDays.test(orig)) {
    d.day(7);
...
parser.prototype.number = function() {
  ...
  if (captures = /^(\d+)/.exec(this.str)) {
    ...
    if (this.date[mod]) {
      ...
      this._meridiem = null;
    } else if (this.original.toLowerCase().match(dateI18n.words.rAt)) {
      this.time(n, 0, 0, this._meridiem);
      this._meridiem = null;
    }
